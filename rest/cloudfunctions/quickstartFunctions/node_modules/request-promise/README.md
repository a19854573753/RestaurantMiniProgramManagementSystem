...
2. Bluebird got updated from v2 to v3. This won't make a difference for most use cases. However, if you use advanced Promise chains starting with the Promise returned by Request-Promise, please check [Bluebird's new features and changes](bluebirdjs.com/docs/new-in-bluebird-3.html).
...
var options = {
    method: 'POST',
    uri: 'api.posttestserver.com/post',
    body: {
        some: 'payload'
    },
...
var options = {
    method: 'DELETE',
    uri: 'my-server/path/to/resource/1234',
...
``` js
var options = {
    uri: 'www.google.com/this-page-does-not-exist.html',
    simple: false    //  <---  <---  <---  <---
};
...
request('google.com', function (err, response, body) {
    if (err) {
...
request.post('example.com/api', function (err, response, body) { ... });
rp.post('example.com/api').then(...);
```
...
For more info on `.then(process).catch(handleError)` versus `.then(process, handleError)`, see Bluebird docs on [promise anti-patterns](bluebirdjs.com/docs/anti-patterns.html#the-.then).
...
This method cancels the request using [Bluebird's cancellation feature](bluebirdjs.com/docs/api/cancellation.html).
...
``` js
// This works:
rp('google.com').then(function () { ... });
rp('google.com').catch(function () { ... });

// This works as well since additional methods are only used AFTER the FIRST call in the chain:
rp('google.com').then(function () { ... }).spread(function () { ... });
rp('google.com').catch(function () { ... }).error(function () { ... });

// Using additional methods as the FIRST call in the chain does not work:
// rp('google.com').bind(this).then(function () { ... });

// Use .promise() in these cases:
rp('google.com').promise().bind(this).then(function () { ... });
```
...
rp({ uri: 'google.com', resolveWithFullResponse: true })
    .then(function (response) {
...
rp({ uri: 'google.com', simple: false })
    .catch(function (reason) {
        // Handle failed request...
	});
...
``` js
var options = {
	uri: 'the-server.com/will-return/404',
	simple: true,
    transform: function (body, response, resolveWithFullResponse) { /* ... */ }
};
...
Based on that you may now build a more sophisticated mock. [Sinon.js](sinonjs.org) may be of help as well.
